...
REAL_EXPERIMENT_NAME="${REAL_EXPERIMENT_NAME:-pccr_transmorph_pkl_v3_vec_full_real_halfrescv_${RUN_ID}}"
JOB_NAME="${JOB_NAME:-pccr_tm_pkl_real_halfrescv}"

JOB_SCRIPT="${JOB_SCRIPT:-${REPO_ROOT}/train_pccr_transmorph_pkl_halfres_final_cv.sh}"
...
if [[ -z "${CHECKPOINT_PATH}" || ! -s "${CHECKPOINT_PATH}" ]]; then
  echo "[full_real_halfrescv] No usable checkpoint found yet under ${SYNTH_CHECKPOINT_ROOT}" >&2
  exit 1
fi
...
echo "=========================================="
echo "PCCR TransMorph PKL Full-Size HalfRes-CV Real Launcher"
...
echo "Final CV downsample factor: ${PCCR_FINAL_CV_DOWNSAMPLE_FACTOR}"
