...
REAL_EXPERIMENT_NAME="${REAL_EXPERIMENT_NAME:-pccr_transmorph_pkl_v3_vec_full_real_fast_${RUN_ID}}"
JOB_NAME="${JOB_NAME:-pccr_tm_pkl_real_full_fast}"
...
# Fast path: let the decoder skip inner chunk checkpoints when outer checkpointing is active.
PCCR_DISABLE_INNER_CHKPT_MAX_VOXELS="${PCCR_DISABLE_INNER_CHKPT_MAX_VOXELS:--1}"
...
if [[ -z "${CHECKPOINT_PATH}" || ! -s "${CHECKPOINT_PATH}" ]]; then
  echo "[full_real_fast] No usable checkpoint found yet under ${SYNTH_CHECKPOINT_ROOT}" >&2
  exit 1
fi
...
echo "=========================================="
echo "PCCR TransMorph PKL Full-Size Fast Immediate Real Launcher"
