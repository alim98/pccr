...
REAL_EXPERIMENT_NAME="${REAL_EXPERIMENT_NAME:-pccr_transmorph_pkl_v3_vec_full_real_now_${RUN_ID}}"
JOB_NAME="${JOB_NAME:-pccr_tm_pkl_real_now_full}"
...
if [[ -z "${CHECKPOINT_PATH}" || ! -s "${CHECKPOINT_PATH}" ]]; then
  echo "[full_real_now] No usable checkpoint found yet under ${SYNTH_CHECKPOINT_ROOT}" >&2
  exit 1
fi
...
echo "=========================================="
echo "PCCR TransMorph PKL Full-Size Immediate Real Launcher"
