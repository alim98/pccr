...
REAL_EXPERIMENT_NAME="${REAL_EXPERIMENT_NAME:-pccr_transmorph_pkl_v3_vec_full_real_opt_${RUN_ID}}"
JOB_NAME="${JOB_NAME:-pccr_tm_pkl_real_full_opt}"
...
# Force inner chunk checkpointing whenever the decoder uses outer checkpointing.
...
if [[ -z "${CHECKPOINT_PATH}" || ! -s "${CHECKPOINT_PATH}" ]]; then
  echo "[full_real_opt] No usable checkpoint found yet under ${SYNTH_CHECKPOINT_ROOT}" >&2
  exit 1
fi
...
echo "=========================================="
echo "PCCR TransMorph PKL Full-Size Optimized Immediate Real Launcher"
