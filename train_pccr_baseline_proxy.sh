...
CONFIG_PATH="${CONFIG_PATH:-src/pccr/configs/pairwise_oasis.yaml}"
CHECKPOINT_PATH="${CHECKPOINT_PATH:-${REPO_ROOT}/checkpoints/pccr/pccr_oasis_real_20260329-100814/epoch1365-val1.5977.ckpt}"
EXPERIMENT_NAME="${EXPERIMENT_NAME:-pccr_baseline_real_proxy_${SLURM_JOB_ID}}"
...
echo "[train_pccr_baseline_proxy.sh] Repository: ${REPO_ROOT}"
echo "[train_pccr_baseline_proxy.sh] Experiment: ${EXPERIMENT_NAME}"
echo "[train_pccr_baseline_proxy.sh] Checkpoint: ${CHECKPOINT_PATH}"
printf '[train_pccr_baseline_proxy.sh] Command:'
printf ' %q' "${CMD[@]}"
printf '\n'
