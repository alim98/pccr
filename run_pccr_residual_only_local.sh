...
EXPERIMENT_NAME="${EXPERIMENT_NAME:-pccr_${VARIANT}_residual_only}"
...
CUDA_VISIBLE_DEVICES="${GPU_ID}" "${PYTHON_BIN}" src/pccr/scripts/train.py \
  ...
  --data_source real \
  --config "${CONFIG_PATH}" \
  --config_override diagnostic_residual_only=true \
  --config_override use_final_residual_refinement=true \
