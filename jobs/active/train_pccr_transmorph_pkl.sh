...
if [[ -n "${SLURM_NTASKS_PER_NODE:-}" && "${SLURM_NTASKS_PER_NODE}" != "${NUM_GPUS_PER_NODE}" ]]; then
  echo "[train_pccr_transmorph_pkl.sh] SLURM_NTASKS_PER_NODE=${SLURM_NTASKS_PER_NODE} does not match NUM_GPUS_PER_NODE=${NUM_GPUS_PER_NODE}." >&2
  exit 1
fi
...
case "${FINAL_COST_VOLUME_MODE}" in
  ...
  off)
    ...
    ;;
  *)
    echo "[train_pccr_transmorph_pkl.sh] Unsupported FINAL_COST_VOLUME_MODE=${FINAL_COST_VOLUME_MODE}. Use one of: streamed, legacy, off." >&2
    exit 1
    ;;
esac
...
echo "[train_pccr_transmorph_pkl.sh] Repository: ${REPO_ROOT}"
echo "[train_pccr_transmorph_pkl.sh] Environment: ${ENV_PATH}"
echo "[train_pccr_transmorph_pkl.sh] Config: ${CONFIG_PATH}"
echo "[train_pccr_transmorph_pkl.sh] Phase: ${PHASE}"
