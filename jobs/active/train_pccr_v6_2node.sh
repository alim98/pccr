...
EXPERIMENT_NAME="${EXPERIMENT_NAME:-pccr_v6_2node_${RUN_TAG}}"
...
case "${FINAL_COST_VOLUME_MODE}" in
  ...
  off)
    ...
    ;;
  *)
    echo "[train_pccr_v6_2node.sh] Unknown FINAL_COST_VOLUME_MODE=${FINAL_COST_VOLUME_MODE}" >&2
    exit 1
    ;;
esac
...
case "${ITER_EVAL_SKIP_HD95,,}" in
  ...
  0|false|no|off)
    ;;
  *)
    echo "[train_pccr_v6_2node.sh] Unknown ITER_EVAL_SKIP_HD95=${ITER_EVAL_SKIP_HD95}" >&2
    exit 1
    ;;
esac
...
echo "[train_pccr_v6_2node.sh] Nodes: ${SLURM_NNODES}, total GPUs: ${TOTAL_GPUS}"
echo "[train_pccr_v6_2node.sh] Config: ${CONFIG_PATH}"
echo "[train_pccr_v6_2node.sh] Checkpoints: ${PCCR_CHECKPOINT_DIR}"
echo "[train_pccr_v6_2node.sh] Aim run: ${AIM_RUN_NAME} (${AIM_RUN_HASH:-new})"
echo "[train_pccr_v6_2node.sh] Phase: ${PHASE}"
printf '[train_pccr_v6_2node.sh] Command:'
printf ' %q' "${CMD[@]}"
printf '\n'
