...
# 2-node variant of train_pccr_transmorph_pkl.sh.
# 8 A100s across 2 nodes via DDP — roughly halves wall-clock time vs single-node.
# Lightning DDP resolves MASTER_ADDR/MASTER_PORT automatically from SLURM env vars.
# NCCL uses the first available network interface; set NCCL_SOCKET_IFNAME if needed.
#
# To use instead of the default single-node script:
...
EXPERIMENT_NAME="${EXPERIMENT_NAME:-pccr_transmorph_pkl_2node_${RUN_TAG}}"
...
case "${FINAL_COST_VOLUME_MODE}" in
  ...
  off)
    ...
    ;;
  *)
    echo "[train_pccr_transmorph_pkl_2node.sh] Unknown FINAL_COST_VOLUME_MODE=${FINAL_COST_VOLUME_MODE}" >&2
    exit 1
    ;;
esac
...
echo "[train_pccr_transmorph_pkl_2node.sh] Nodes: ${SLURM_NNODES}, total GPUs: ${TOTAL_GPUS}"
echo "[train_pccr_transmorph_pkl_2node.sh] Config: ${CONFIG_PATH}"
echo "[train_pccr_transmorph_pkl_2node.sh] Checkpoints: ${PCCR_CHECKPOINT_DIR}"
echo "[train_pccr_transmorph_pkl_2node.sh] Aim run: ${AIM_RUN_NAME} (${AIM_RUN_HASH:-new})"
echo "[train_pccr_transmorph_pkl_2node.sh] Phase: ${PHASE}"
printf '[train_pccr_transmorph_pkl_2node.sh] Command:'
printf ' %q' "${CMD[@]}"
printf '\n'
